...
if [[ -f "$dc" ]] && grep -q 'extraMounts:' "$patched" && ! grep -q 'containerPath: /var/lib/kubelet/config.json' "$patched"; then
    ...
    indent="$(awk '/extraMounts:/ { f=1; next } f && /- hostPath:/ { match($$0,/^[[:space:]]*/); print substr($$0,RSTART,RLENGTH); exit }' "$patched")"

    # fallback indent if detection fails
    [[ -z "$indent" ]] && indent="      "

    # inject mount after extraMounts:
    awk -v dc="$dc" -v ind="$indent" '
    BEGIN { added=0 }
    /^[[:space:]]*extraMounts:/ && !added {
        print
    ...
    echo ">> patch-kind-config: injected docker config mount"
